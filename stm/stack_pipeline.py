...
#         self.data_path='/dfs/data/data/tc/'
        ...
        self.threshold=configs['model']['threshold']
...
#         for module in model.wit_list:
#             wit_list_params += list(module.parameters())

        # 剩余的模型参数（排除 wit_list）
#         other_params = [param for name, param in model.named_parameters() if name not in set(p for p in model.wit_list.parameters())]
...
#         return recon_loss + kl_loss
    ...
    def step_calc_loss(self, model, data: tuple, weight=None, **kwargs) -> torch.Tensor:
        ...
        class_labels = torch.zeros_like(y_true, dtype=torch.long)
        for i, threshold in enumerate(thresholds):
            class_labels += (y_true > threshold).long()
        if signal == 'class':
            class_pred,_ = model(x, pad_mask=pad_mask, signal='class')
            y_binary = (y_true > 0).long()
            criterion = nn.CrossEntropyLoss()
            binary_loss = criterion(class_pred[~ignore_index].view(-1, class_pred.shape[-1]),class_labels[~ignore_index].view(-1))
            return binary_loss, class_pred
        else:
            pred,index = model(x, pad_mask=pad_mask, signal=signal,class_true=class_labels)
#             print(pred.shape,y_true[index].shape)
            loss = F.mse_loss(pred, y_true[index])
            return loss, index,pred
#         elif signal == 'pos':
...
#         return torch.zeros(y_true.shape[0]).to(y_true), pos_loss, neg_loss, y_pred
    def pn_calc_loss(self,model,data,weight=None,**kwargs):
        x, y_true, pad_mask, ignore_index,time= data
        thresholds=self.threshold
        class_labels = torch.zeros_like(y_true, dtype=torch.long)
        for i, threshold in enumerate(thresholds):
        ...
        y_pred,class_pred = model(x, class_true=class_labels,pad_mask=pad_mask)
#         print(class_pred.shape,class_labels.shape)
        criterion = nn.CrossEntropyLoss()
        class_loss=criterion(class_pred.view(-1,len(thresholds)+1),class_labels.view(-1))
#         for name, param in model.named_parameters():
#             print(f"Parameter: {name}")
#             print(f"Gradient: {param.grad}")
        if weight is not None:
            mse_loss=torch.mean(weight*(y_true-y_pred)**2)
        else:
...
#         mse_loss.backward()
        ...
        return class_loss,mse_loss,sample_loss,y_pred
...
#                     weight=torch.load('mse_valid_weight.pt').to(self.local_rank)
                    optimizer.zero_grad()
                    if self.scheme=='pn':
                        class_loss,mse_loss,sample_loss,y_pred=self.loss_func(model,data,weight)
...
#                         mse_loss=mse_loss.detach()
                        ...
                        y_pred=torch.zeros(1,3759).to(self.local_rank)
                        class_loss,class_pred=self.step_calc_loss(model,data,signal='class')
                        optimizer.zero_grad()
                        class_loss.backward()
                        optimizer.step()
                        for i in range(len(self.threshold)+1):
                            optimizer.zero_grad()
                            loss,index,pred=self.step_calc_loss(model,data,signal=i)
                            loss.backward()
                            optimizer.step()
                            y_pred[index]=pred
